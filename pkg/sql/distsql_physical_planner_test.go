	...
	for testIdx, tc := range testCases {
		t.Run(strconv.Itoa(testIdx), func(t *testing.T) {
			...
			for i, p := range partitions {
				...
				if userData != nil {
					userDataMap[int(p.Node)] = userData[i]
				}
			}
