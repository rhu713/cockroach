...
func (sc *SchemaChanger) Merge(
	...
	return sc.db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		// TODO(rhu): how to set the timestamp? do we need to set timestamp?
		...
		return txn.Iterate(ctx, sourceSpan.Key, sourceSpan.EndKey, pageSize, func(kvs []kv.KeyValue) error {
			...
			for i := range kvs {
				...
				if deleted {
					fmt.Printf("rh_debug: deleting k=%v\n", mergedEntry.Key)
					wb.Del(mergedEntry.Key)
				} else {
					fmt.Printf("rh_debug: putting k=%v v=%v vbytes=%v\n", mergedEntry.Key, mergedEntry.Value,
						mergedEntry.Value.RawBytes)
