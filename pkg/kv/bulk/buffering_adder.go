...
func (b *BufferingAdder) Close(ctx context.Context) {
	if log.V(1) {
		if b.sink.totalStats.BufferFlushes > 0 {
			b.sink.totalStats.LogTimings(ctx, b.name, "closing")
			if log.V(3) {
				b.sink.totalStats.LogPerStoreTimings(ctx, b.name)
			}
			b.sink.totalStats.LogFlushes(ctx, b.name, "closing", b.memAcc.Used())
		} else {
...
func (b *BufferingAdder) Add(ctx context.Context, key roachpb.Key, value []byte) error {
	...
	b.sink.totalStats.FlushesDueToSize++
...
func (b *BufferingAdder) doFlush(ctx context.Context, forSize bool) error {
	b.sink.totalStats.FillWait += timeutil.Since(b.lastFlush)
	...
	b.sink.totalStats.BufferFlushes++

	before := b.sink.totalStats
	...
	b.sink.totalStats.SortWait += beforeFlush.Sub(beforeSort)
	...
	if log.V(1) {
		if len(b.sink.totalStats.Span.Key) == 0 || b.curBuf.Key(0).Compare(b.sink.totalStats.Span.Key) < 0 {
			b.sink.totalStats.Span.Key = b.curBuf.Key(0).Clone()
		}
	}
	...
	if log.V(1) {
		if b.sink.totalStats.Span.EndKey.Compare(mvccKey.Key) < 0 {
			b.sink.totalStats.Span.EndKey = mvccKey.Key.Clone()
		}
	}

	b.sink.totalStats.FlushWait += timeutil.Since(beforeFlush)

	if log.V(3) {
		written := b.sink.mu.totalRows.DataSize - beforeSize
		files := b.sink.totalStats.Batches - before.Batches
		dueToSplits := b.sink.totalStats.BatchesDueToRange - before.BatchesDueToRange
		dueToSize := b.sink.totalStats.BatchesDueToRange - before.BatchesDueToRange
	...
	if log.V(2) {
		b.sink.totalStats.LogTimings(ctx, b.name, "flushed")
		if log.V(3) {
			b.sink.totalStats.LogPerStoreTimings(ctx, b.name)
		}
	}
	...
	if log.V(3) {
		b.sink.totalStats.LogFlushes(ctx, b.name, "flushed", b.memAcc.Used())
	}
...
func (b *BufferingAdder) createInitialSplits(ctx context.Context) error {
	...
	b.sink.totalStats.Splits += int64(len(toScatter))
	b.sink.totalStats.SplitWait += splitsWait
	...
	for _, splitKey := range toScatter {
		...
		b.sink.totalStats.Scatters++
		b.sink.totalStats.ScatterMoved += resp.ReplicasScatteredBytes
	...
	b.sink.totalStats.ScatterWait += scattersWait
