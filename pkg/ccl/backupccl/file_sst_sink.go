...
func (s *fileSSTSink) write(ctx context.Context, resp exportedSpan) error {
	...
	if l := len(s.flushedFiles) - 1; l > 0 {
		log.Infof(ctx, "rh_debug: new file span=%v start=%v end=%v, last file span=%v start=%v end=%v",
			resp.metadata.Span, resp.metadata.StartTime, resp.metadata.EndTime, s.flushedFiles[l].Span,
			s.flushedFiles[l].StartTime, s.flushedFiles[l].EndTime)
	}
